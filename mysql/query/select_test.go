...
func TestSelectBasicQuery(t *testing.T) {
	...
	testSelectBuilder(t, "SELECT WITH WHERE AND OR NESTED",
		...
		"SELECT `p`.`createdAt`, `p`.`updatedAt`, `p`.`id`, `p`.`fullName` FROM `Person` AS `p` WHERE (`p`.`fullName` LIKE ? OR `p`.`fullName` NOT LIKE ? OR (`p`.`id` IN (?) AND `p`.`id` NOT IN (?, ?))) AND (`p`.`id` = ? AND `p`.`id` != ? AND `p`.`id` < ? AND `p`.`id` <= ? AND `p`.`id` > ? AND `p`.`id` >= ? AND (`p`.`createdAt` BETWEEN ? AND ? OR `p`.`updatedAt` NOT BETWEEN ? AND ?))",
	)
}
